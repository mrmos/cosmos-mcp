...
URL: https://api.cosmos.so/graphql?q=GetAllElements

## Query
```graphql
query GetAllElements($userId: UserId!, $filters: AllElementsFilters, $order: ElementOrder, $pageCursor: String, $pageSize: Int = 40, $isLoggedIn: Boolean!) {
  allElementsV2(
    userId: $userId
    filters: $filters
    order: $order
